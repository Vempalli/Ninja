...
public class FindNumberOfUsers<Integer> {
	...
	private class Node {
		...
		//int N;
		int max;
		Node (LogTime time, Integer label) {
			timeInt = time;
			this.label = label;
			this.max = time.end;
		}	
	...
	public Integer get(LogTime time) {
		return get(root, time);
	}
	private Integer get(Node node, LogTime time) {
		if (node == null)
			return null;
	 ...
	 * insertion
	 ***********************/
	public void put(LogTime time, Integer label) {
		if (contains(time)) {
	...
	private Node insert(Node node, LogTime time, Integer label) {
		if (node == null) {
	...
	public static void main(String[] args) {
		int N = 10;
		FindNumberOfUsers fu = new FindNumberOfUsers ();
		for (int i = 0; i < N; i++) {
